...
class BasicLayer(nn.Module):
    ...
    def create_mask(self, x, H, W):
        ...
        # [1,49,49]
        # print("mask_size:{}".format(attn_mask.shape))
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000,
        ...
        # 定义classifier
        # 似乎不应该在这里定义classifier，timm中并没有在这里定义
    ...
    def forward_features(self,x):
        ...
        x = self.pos_drop(x)
        # [32,3136,128]
        # print("pos_drop后:{}".format(x.shape))
        for layer in self.layers:
            x, H, W = layer(x, H, W)
        # [32,49,1024]
        # print("norm前:{}".format(x.shape))
        x = self.norm(x)  # [B, L, C]
    ...
    def forward(self, x):
        ...
        # print("head前：{}".format(x.shape))
        x = self.head(x)
        # print("head后：{}".format(x.shape))
        return x
